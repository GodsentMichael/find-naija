...
describe('Search API Routes', () => {
  ...
  it('Should search regions', async () => {
    ...
    expect(res.body).toEqual(expect.arrayContaining(['North Central', 'North East', 'North West', 'South East',
    'South South',
    'South West']));
  });
  ...
  it('Should search states by region', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body).toEqual(expect.arrayContaining(['Abia', 'Anambra', 'Ebonyi','Enugu', 'Imo']));
  });
